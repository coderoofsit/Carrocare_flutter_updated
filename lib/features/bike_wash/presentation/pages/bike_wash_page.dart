...
class _BikeWashPageState extends State<BikeWashPage> {
  ...
  Widget build(BuildContext context) {
    return CarroCareScaffold(
      title: 'Daily Bike Wash',
      onBack: () => context.pop(),
      backgroundDecoration: const BoxDecoration(
        gradient: AppGradients.washScreenBackground,
      ),
      footer: BlocBuilder<BikeWashBloc, BikeWashState>(
        builder: (context, state) {
          final price =
              state is BikeWashLoaded ? state.service.displayPrice : '';
          return Container(
            height: 100,
      ...
      body: BlocBuilder<BikeWashBloc, BikeWashState>(
        builder: (context, state) {
